...
<div class="payment-container">
    ...
    <div class="panel-card">
        <h2 class="payment-title">Detail Pemesanan</h2>
        ...
        <div style="text-align: center; margin-bottom: 25px;">
            ...
            <h3 style="color: white; margin: 0; font-size: 20px;">{{ $appointment->service->name }}</h3>
            <p style="color: #d8b67a; font-weight: bold; font-size: 24px; margin: 8px 0 0 0;">
                Rp {{ number_format($appointment->service->price, 0, ',', '.') }}
            </p>
        </div>

        <div class="detail-item">
            <span class="detail-label">Stylist</span>
            <span class="detail-value">{{ $appointment->stylist->name }}</span>
        </div>
    ...
    <div class="panel-card">
        <h2 class="payment-title">Buat Pembayaran</h2>
        ...
        <form action="{{ route('payments.store') }}" method="POST" enctype="multipart/form-data" id="payment_form">
            @csrf
            <input type="hidden" name="appointment_id" value="{{ $appointment->id }}">
            <input type="hidden" name="payment_method" id="payment_method_input" value="{{ old('payment_method', 'transfer') }}">
            ...
            <div class="input-group">
                ...
                <div class="method-selector">
                    {{-- Transfer Bank --}}
                    <div class="method-card {{ old('payment_method', 'transfer') === 'transfer' ? 'active' : '' }}" data-method="transfer">
                    ...
                    {{-- QRIS --}}
                    <div class="method-card {{ old('payment_method') === 'qris' ? 'active' : '' }}" data-method="qris">
                        <svg viewBox="0 0 24 24">
                    ...
                    {{-- Credit Card --}}
                    <div class="method-card {{ old('payment_method') === 'card' ? 'active' : '' }}" data-method="card">
                        <svg viewBox="0 0 24 24">
            ...
            <div id="instruction_transfer" class="instruction-box {{ old('payment_method', 'transfer') === 'transfer' ? 'active' : '' }}">
                ...
                <div style="margin-top: 20px;">
                    ...
                    <input type="text" name="bank_account" id="bank_account_input" value="{{ old('bank_account') }}" placeholder="Contoh: BCA - Ahmad Fauzi" class="input-control">
                </div>
            </div>

            {{-- Instruction QRIS --}}
            <div id="instruction_qris" class="instruction-box {{ old('payment_method') === 'qris' ? 'active' : '' }}">
            ...
            {{-- Instruction Card --}}
            <div id="instruction_card" class="instruction-box {{ old('payment_method') === 'card' ? 'active' : '' }}">
            ...
            <div class="input-group">
                ...
                <input type="number" name="amount" id="amount_input" value="{{ old('amount', $appointment->service->price) }}" step="0.01" required class="input-control">
            </div>

            {{-- 4. PROOF OF PAYMENT UPLOAD WITH PREVIEW --}}
            <div class="input-group">
                <label class="input-label">Unggah Bukti Transfer / Pembayaran *</label>
                <div class="file-upload-zone" onclick="document.getElementById('proof_image_input').click()">
                    ...
                    <p style="color: white; font-weight: bold; margin: 0 0 6px 0; font-size: 14px;">Klik untuk Unggah Gambar</p>
                    <p style="color: #999; margin: 0; font-size: 12px;">Format: JPEG, PNG, JPG, GIF (Max 2MB)</p>
                    
                    <input type="file" name="proof_image" id="proof_image_input" accept="image/*" required style="display: none;">
                </div>
                
                {{-- Live Preview Container --}}
                <div id="preview_wrapper" class="preview-container">
                    <p style="color: #d8b67a; font-size: 13px; font-weight: bold; margin: 0 0 10px 0;">Pratinjau Bukti Transfer:</p>
                    <img id="image_preview" src="#" alt="Pratinjau Bukti" class="preview-image">
                </div>
            </div>
            ...
            <div class="input-group">
                ...
                <textarea name="notes" id="notes_input" rows="3" placeholder="Tambahkan catatan jika diperlukan..." class="input-control" style="font-family: 'Poppins', sans-serif; resize: vertical;">{{ old('notes') }}</textarea>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 30px;">
                ...
                <a href="{{ route('dashboard') }}" class="action-btn btn-secondary">
                    Batal
                </a>
            </div>
        </form>
